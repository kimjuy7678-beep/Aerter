...
// Production: replace handleSocialLogin with real OAuth redirects.
// e.g. Google: window.location.href = `https://accounts.google.com/o/oauth2/v2/auth?client_id=...`
// Kakao: Kakao.Auth.authorize({ redirectUri: ... })
// Naver: window.location.href = `https://nid.naver.com/oauth2.0/authorize?...`

const PROVIDERS = [
  {
    id: 'kakao' as SocialProvider,
    label: '카카오로 계속하기',
    bg: 'bg-[#FEE500]',
    text: 'text-[#191919]',
    ...
    mockUser: { name: '카카오 사용자', email: 'user@kakao.com' },
  },
  {
    id: 'naver' as SocialProvider,
    label: '네이버로 계속하기',
    bg: 'bg-[#03C75A]',
    text: 'text-white',
    ...
    mockUser: { name: '네이버 사용자', email: 'user@naver.com' },
  },
  {
    ...
    mockUser: { name: 'Google 사용자', email: 'user@gmail.com' },
  },
];
...
export default function LoginPage() {
  ...
  // Production: this function should redirect to the OAuth provider's auth URL.
  // The OAuth callback URL should call login() after receiving the access token.
  const handleSocialLogin = (provider: typeof PROVIDERS[number]) => {
    login({
      id: `${provider.id}-mock-${Date.now()}`,
      name: provider.mockUser.name,
      email: provider.mockUser.email,
        ...
        <div className="flex flex-col gap-3">
          {PROVIDERS.map((provider) => (
            ...
            >
              ...
              {/* Label — centered */}
              <span className="w-full text-center">{provider.label}</span>
            </button>
          ))}
        </div>
